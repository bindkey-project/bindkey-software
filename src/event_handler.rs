...
use crate::protocol::protocol::{
    ApiMessage, BindKeyInfo, LoginSuccessResponse, Page, RegisterPayload, Role, StatusBindkey,
    User, UserWithBindKey, VolumeCreatedInfo, VolumeInfo, VolumeTab,
};
...
pub fn handle_api_message(app: &mut BindKeyApp, message: ApiMessage) {
    match message {
        ...
        ApiMessage::EnrollmentUsbSuccess(data) => {
            if let UsbResponse::Success(SuccessData::EnrollmentInfo {
                sn,
                pub_sign,
                pub_ecdh,
            }) = data
            {
                app.enroll_status =
                    "Données matérielles récupérées, envoi au serveur...".to_string();
                ...
                tokio::spawn(async move {
                    ...
                    match res {
                        ...
                        _ => {
                            let _ = clone_sender
                                .send(ApiMessage::EnrollmentError("Refus serveur".into()));
                        }
                    }
                });
        ...
        ApiMessage::ReceivedChallenge(challenge, session_id, sn) => {
            ...
            tokio::spawn(async move {
                if !clone_port.is_empty() {
                    if let Ok(mut port) = serialport::new(&clone_port, 115200)
                        ...
                        match send_text_command(&mut *port, &cmd) {
                            Ok(map) => {
                                if let Some(sig) = map.get("SIG") {
                                    ...
                                    let _ = clone_sender
                                        .send(ApiMessage::LoginError("Signature absente".into()));
                                }
                            }
                            Err(e) => {
                                let _ = clone_sender
                                    .send(ApiMessage::LoginError(format!("Erreur USB: {}", e)));
                            }
                        }
                    }
                }
        ...
        ApiMessage::SignedChallenge(signature, session_id, sn) => {
            ...
            tokio::spawn(async move {
                ...
                let _ =
                    clone_sender.send(ApiMessage::LoginError("Authentification échouée".into()));
            });
        }
        ...
        ApiMessage::VolumeInfoReceived(data) => {
            if let UsbResponse::Success(SuccessData::DeviceInfo {
                device_name,
                device_size,
                    ...
                    Ok(resp) => {
                        ...
                        if status.is_success() {
                            ...
                            let error_text = resp
                                .text()
                                .await
                                .unwrap_or_else(|_| "Détails indisponibles".to_string());
        ...
        ApiMessage::UpdateBindKeyStatus(sn, status) => {
            ...
            tokio::spawn(async move {
                let _ = clone_api_client
                    .put(format!("{}/bindkeys/status", url))
                    .json(&serde_json::json!({"serial_number": sn, "status": status}))
                    .bearer_auth(clone_auth_token)
                    .send()
                    .await;
                    ...
                    if let Ok(map) = send_text_command(&mut *port, "action=init_format\n") {
                        ...
                        if map.get("STATUS").map(|v| v.contains("OK")).unwrap_or(false) {
                            let _ = tokio::task::spawn_blocking(move || {
                                crate::pages::volumes::force_format(&device_path, &partitions)
                            })
                            .await;
                            let _ = clone_sender.send(ApiMessage::FormatStatus(
                                "Succès : Clé réinitialisée.".to_string(),
                            ));
                            let _ = clone_sender.send(ApiMessage::RequestVolumeRefresh);
                        }
                    }
            ...
            tokio::spawn(async move {
                if let Ok(output) = Command::new("lsblk")
                    .args(&[
                        ...
                        "-b",
                        "-o",
                        "NAME,MODEL,SIZE,TRAN,FSTYPE,PTTYPE,MOUNTPOINT,LABEL,FSUSED",
                    ])
                    .output()
                {
                    let ouput_str = String::from_utf8_lossy(&output.stdout);
                    if let Ok(parsed) =
                        serde_json::from_str::<crate::protocol::protocol::LsblkOutput>(&ouput_str)
                    {
                                    ...
                                    for part in children {
                                        ...
                                        if let Some(label) = part.label {
                                            extracted_volumes.push(VolumeInfo {
                                                ...
                                                total_space_gb: ((part.size as f64
                                                    / 1_073_741_824.0)
                                                    * 10.0)
                                                    .round()
                                                    / 10.0,
                                                used_space_gb: part.fsused.as_ref().and_then(|v| {
                                                    v.as_u64().map(|b| {
                                                        ((b as f64 / 1_073_741_824.0) * 10.0)
                                                            .round()
                                                            / 10.0
                                                    })
                                                }),
        ...
        ApiMessage::StartVolumeDeletion(name, path) => {
            ...
            tokio::spawn(async move {
                ...
                let _ =
                    clone_sender.send(ApiMessage::VolumeDeletionError("ID non trouvé".to_string()));
            });
        }
        ApiMessage::VolumeIdReceivedForDeletion(name, id, path) => {
            ...
            tokio::spawn(async move {
                if !clone_port.is_empty() {
                    ...
                    {
                        let _ = port.write_data_terminal_ready(true);
                        if send_text_command(&mut *port, &format!("delete_volume={}", clone_id))
                            .is_ok()
                        {
...
"#;
        ...
        ApiMessage::VolumeDeletedOnServer(id) => {
            ...
            tokio::spawn(async move {
                let _ = clone_api_client
                    .delete(format!("{}/volumes/{}", clone_url, clone_id))
                    .bearer_auth(&clone_token)
                    .send()
                    .await;
                let _ = clone_sender.send(ApiMessage::VolumeDashboardStatus(
                    "Volume supprimé !".to_string(),
                ));
                let _ = clone_sender.send(ApiMessage::RequestVolumeRefresh);
            });
        }
        ApiMessage::VolumeDeletionError(err) => {
            app.dashboard_status = format!("❌ {}", err);
            app.is_loading = false;
        }
        ...
        ApiMessage::SearchUserError(err) => {
            app.search_result = None;
            app.enroll_status = err;
            app.is_loading = false;
        }
        ApiMessage::UpdateBindKeyError(err) => {
            app.enroll_status = format!("❌ {}", err);
        }
        ApiMessage::LogOutError(err) => println!("LogOut Error: {}", err),
        ApiMessage::DeleteUserError(err) => {
            app.enroll_status = format!("❌ {}", err);
        }
        ApiMessage::FetchUsersError(err) => {
            app.enroll_status = format!("❌ {}", err);
        }
        _ => {}
    }
