...
pub fn get_bindkey(app: &mut BindKeyApp, ui: &mut egui::Ui)  -> Result<String, String> {
     app.enroll_status = "🔌 Recherche de la clé USB...".to_string();
            ...
            if bypass_usb {
                ...
                resultat_usb = Ok(r#"{"status": "SUCCESS"}"#.to_string());
            } else {
                app.devices.clear();
                ...
                if let Some(device) = app.devices.first() {
                    resultat_usb =
                        send_command_bindkey(&device.port_name, Command::StartEnrollment);
                } else {
